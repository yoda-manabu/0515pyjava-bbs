...
<body>
    ...
    %>
        <p>
        	[<%= r.getCreatedAt() %>] <%= r.getAnonId() %>: <%= r.getContent() %>
        
	<!-- 削除フォーム -->
	
	<form action="<%= request.getContextPath() %>/reply/delete" method="post" style="display:inline;">
    	<input type="hidden" name="replyId" value="<%= r.getId() %>">
...
<p><strong>投稿日:</strong> <%= post.getCreatedAt() %></p>
<p><strong>投稿者:</strong> <%= post.getDisplayName() %></p>
...
%>
<p>
[<%= r.getCreatedAt() %>] <%= r.getDisplayName() %>: <%= r.getContent() %>
